...
Last updated: 2026-09-16. Prefer reading this file over broad codebase exploration when adding dashboards or Homey features.
...
| **Sonos**                   | `src/components/Devices/Sonos/*`           | Shared `SonosFocus` on `/` + `/andre`. Stue library = Sonos favorites per selected speaker (`cloud_play_sonos_favorite`). Library uses React `Suspense` + `FavoritesSkeleton` (same 100px tile grid) so loading does not shift layout. Container query: two columns in landscape overlay (Stue 800×480); stacked in portrait (`/andre`). `deviceId` + `embedded` + `onBackClick` / `onShowLibrary`                                                                                                                                              |
| **Spotify Connect**         | `SonosFocus` + `SpotifyPlaylists`          | `/andre` Fabian via `speakerDeviceId` → `nl.pendo.spotify` (unchanged). Stue list includes the same device; playlists via flow card `play_playlist` (Homey/API order ≈ iOS Your Library **Nylige**). Same `Suspense` skeleton as Sonos favorites.                                                                                                                                                                                                                                                                                               |
