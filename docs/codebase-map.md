...
Last updated: 2026-09-23. Prefer reading this file over broad codebase exploration when adding dashboards or Homey features.

## Stack

- Vite + React 19.3 + `react-router-dom` 7
...
| --------------------------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **All speakers** (Stue `/`) | `src/components/Devices/Speakers/*`        | Auto-discovers Homey devices with `speaker_playing`. Overlay: list (cover + left-bleed blur) → player → library in one `FocusedElement` shell. React 19 `<ViewTransition>` + `addTransitionType` for forward/back slides; shared-element morph of speaker name (row → title) and album art both ways. List stays mounted (parked) while the player is open so reverse morphs reuse the same decoded cover. List cover `src` is passed into `SonosFocus` as `initialCoverUrl`. Homey names. No grouping. Refresh device list when overlay opens.                                                                                                                                                        |
| **Sonos**                   | `src/components/Devices/Sonos/*`           | Shared `SonosFocus` on `/` + `/andre`. Stue library = Sonos favorites per selected speaker (`cloud_play_sonos_favorite`). Prefetch starts when the speakers overlay opens; a fresh Homey fetch is skipped for 5 minutes after a successful one. Last list is also in `localStorage`, so the grid can render immediately and refresh behind it. Library uses React `Suspense` + `FavoritesSkeleton` (same 100px tile grid). Player transport uses the device already on the list; until that object exists, prev/play/next render disabled. Container query: two columns in landscape overlay (Stue 800×480); stacked in portrait (`/andre`). `deviceId` + `embedded` + `onBackClick` / `onShowLibrary` |
| **Spotify Connect**         | `SonosFocus` + `SpotifyPlaylists`          | `/andre` Fabian via `speakerDeviceId` → `nl.pendo.spotify` (unchanged). Stue list includes the same device; playlists via flow card `play_playlist` (Homey/API order ≈ iOS Your Library **Nylige**). Prefetch on overlay open, same 5-minute skip after a live fetch. `localStorage` shows the last list at once; while the library is open, a failed or cached list retries every 10s. Same `Suspense` skeleton as Sonos favorites.                                                                                                                                                                                                                                                                   |
